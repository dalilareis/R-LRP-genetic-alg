...
totalCost <- function(chromossome, distClients, distDepot, costDepot, demands, depotCapacity){
  ...
  if (sizeChromossome1 >= 200) {
    depots <- matrix(chromossome, nrow = floor(avgClients * maxCarDepot / 3), ncol = numDep)
  }
  if (sizeChromossome1 < 200) {
  ...
  for(depot in seq_along(depots[1, ])){
    ...
    removeCli <- numCli + 1:sizeChromossome #Remove numbers > numCli from tour
  ...
  if (sizeChromossome1 >= 200) {
    routes <- matrix(chromossome, nrow = avgClients, ncol = floor(maxCarDepot * numDep / 3))
  }
  if (sizeChromossome1 < 200) {
  ...
  for(car in seq_along(routes[1, ])){
    ...
    if(length(tour) != 0) {
      ...
      if (totalOrder > carCapacity) {
        penalty <- penalty + (100*costDepot[depotIndex, ])
      }
...
for (i in 1:files ) {
  ...
  sizeChromossome1 <- maxRotas * avgClients
  ...
  #If each depot has a lot of cars (Tuzun: 1 depot can supply all clients)
  if (sizeChromossome1 >= 200) {
    sizeChromossome <- floor(sizeChromossome1 / 3) #simulate less cars for each depot
  }
  
  if (sizeChromossome1 < 200) {
    sizeChromossome <- sizeChromossome1
  }
  ...
  if (sizeChromossome1 >= 200) {
    carDepot <- sliceChrom(chromossome, (floor(maxCarDepot * avgClients / 3)))
    usedDep <- 0
    for (j in 1:length(carDepot)){
      if (any(carDepot[[j]] <= numCli)){
        usedDep <- usedDep + 1
  ...
  if (sizeChromossome1 >= 200) {
    car <- sliceChrom(chromossome, (avgClients/3))
    usedCars <- 0
    for (j in 1:length(carDepot)){
      if (any(car[[j]] <= numCli)){
        usedCars <- usedCars + 1
...
write.xlsx(results, file = "Results.xlsx", sheetName = "AvgRotas", col.names = T, 
